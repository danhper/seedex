defmodule Seedex do
  ...
  defp make_changeset(record, existing) do
    associations = record.__struct__.__schema__(:associations)
    fields = record.__struct__.__schema__(:fields)

    loaded_associations =
      Enum.filter(associations, fn key ->
        value = Map.get(record, key)
        not is_nil(value) and not match?(%Ecto.Association.NotLoaded{}, value)
      end)

    existing = repo().preload(existing, loaded_associations)
    changes = Map.take(record, fields ++ associations)

    Enum.reduce(changes, Ecto.Changeset.change(existing), fn
